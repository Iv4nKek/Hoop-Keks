...
namespace Code.Core
{
    ...
    public class ImagePublisher 
    {
        ...
        private static string clientID = "5571092";
        private static string clientSecret ="UMeKEtvmUIZiQiC3eui3";

        public static void Main()
        {
            Application.
            WebRequest request = WebRequest.Create(" https://oauth.vk.com/authorize?client_id=5571092&display=page&scope=friends&response_type=token&v=5.130&state=123456");
            ...
            VK_GetServerToken(clientID, clientSecret);
            VK_GetUserAutorization(token);
        }
        ...
        public static SocialProfile VK_GetUserAutorization(string token)
        {
            SocialProfile profile = new SocialProfile();

            string str = GetRequest("api.vk.com", "https://api.vk.com/method/secure.checkToken?token=" + token + "&v=5.85&client_secret="+clientSecret+"&access_token="+token);
        ...
        public static SocialProfile VK_GetUserData(string token, SocialProfile profile)
        {
            try
            {
